...
def load_obf(file_path):

    obf = obf_support.File(file_path)

    outputDict = {}
    outputDictMeta = {}
    for index, stack in enumerate(obf.stacks):
        outputDictMeta[stack.name]={}
        # #         print('\nstack {}'.format(index))
        ...
        data = stack.data
        #print(data.shape)
        idx = [slice(None), slice(None), slice(None)] + [0] * (len(data.shape) - 3)
        outputDict[stack.name] =data[tuple(idx)].reshape(data.shape[:3]) 
        outputDictMeta[stack.name]["px"]=stack.pixel_sizes
        #outputDictMeta[stack.name]["offset"]=stack.offsets
...
class OBFReader:
    ...
    def read(self, obffile):
        obf = obf_support.File(obffile)
        data = {}
        for stack in obf.stacks:
            data[stack.name] = stack.data
        return data
    
    def get_metadata(self, obffile):
        obf = obf_support.File(obffile)
        data = {}
        for stack in obf.stacks:
            data[stack.name] = stack.metadata
        return data
